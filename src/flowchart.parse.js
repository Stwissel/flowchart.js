...
function parse(input) {
    ...
    function getStyle(s) {
        ...
        if (startIndex >= 0 && endIndex >= 0) {
            var candidate = s.substring(startIndex, endIndex);
            return (candidate.indexOf('{') < 0) ? candidate : JSON.parse(candidate);
        }
        return {};
    }
    ...
    while (lines.length > 0) {
        ...
        if (line.indexOf('=>') >= 0) {
            ...
            for (var j = 0, lenSy = lineStyleSymbols.length; j < lenSy; j++) {

                if ((j + 1) != lenSy) {
                    var curSymb = getSymbol(lineStyleSymbols[j]);
                    var nxtSymb = getSymbol(lineStyleSymbols[j + 1]);

                    curSymb.lineStyle[nxtSymb.key] = getStyle(lineStyleSymbols[j + 1]);
                }
            }
        }
