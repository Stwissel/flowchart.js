...
Symbol.prototype.renderLines = function(allOptions) {
    if (this.next) {
...
Symbol.prototype.drawLineTo = function(symbol, text, origin, allOptions) {
    // TODO: Harden function to support variable number or arguments
    ...
    if ((!origin || origin === 'bottom') && isOnSameColumn && isUnder) {
        ...
        line = drawLine(this.chart, right, [{ x: symbolTop.x, y: right.y }, { x: symbolTop.x, y: symbolTop.y }], text);
        this.rightStart = true;
        symbol.topEnd = true;
        maxX = right.x + lineLength / 2;
    ...
    if (this.lineStyle[symbol.key] && line) {
        var candidate = this.lineStyle[symbol.key];
        if (typeof candidate === 'object') {
            line.attr(candidate);
        } else if (typeof candidate === 'string' && allOptions.lineStyle[candidate]) {
            line.attr(allOptions.lineStyle[candidate]);
        }
    }
